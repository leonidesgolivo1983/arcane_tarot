...
function drawCards() {
  ...
  // Reproducir la narración con voz misteriosa
...
// Función para narrar el significado con voz misteriosa
function speakNarrative(text) {
  if ('speechSynthesis' in window) {
    const utterance = new SpeechSynthesisUtterance(text);
    utterance.lang = "es-ES"; // Español
    utterance.pitch = 0.8;    // Tono profundo y grave
    utterance.rate = 0.8;     // Ritmo pausado y controlado
    utterance.volume = 1.0;
    speechSynthesis.speak(utterance);
  } else {
    console.log("Tu navegador no soporta Speech Synthesis.");
  }
}
